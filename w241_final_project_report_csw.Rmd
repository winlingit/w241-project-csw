...
dt=fread('~/Documents/Berkeley/241/Final\ Project/w241-project-csw/rcode/atg_results.csv')
dt[Female == 0, Sex := 'M']
dt[Female == 1, Sex := 'F']
dt[, Y := Responses / N]
```
...
```{r}
atg.stats = dt[order(Org, Region, Sex, treat), .(Org, Region, Sex, Treat=treat, N)]
kable(atg.stats, digits=2)
```

```{r}
atg.results = dcast(dt, 
...
```{r}
dt=fread('~/Documents/Berkeley/241/Final\ Project/w241-project-csw/rcode/ptg_assignment.csv')
dt[Female == 0, Sex := 'M']
dt[Female == 1, Sex := 'F']
dt = dt[order(Org, Region, Sex, Treat), .(Org, Region, Sex, Treat, N)]
```
