...
pilot.perms = genperms(dt.pilot$treat, maxiter=RI_ITERATIONS)
pilot.probs = genprobexact(dt.pilot$treat)
pilot.ate = estate(dt.pilot$responded, dt.pilot$treat, prob=pilot.probs); pilot.ate

# Getting p-value under sharp null
pilot.Ys.null <- genouts(dt.pilot$responded, dt.pilot$treat, ate=0) # generate potential outcomes under sharp null of no effect
pilot.distout.null <- gendist(pilot.Ys.null, pilot.perms, prob=pilot.probs) # generate sampling dist. under sharp null
pilot.p = dispdist(pilot.distout.null, pilot.ate)$two.tailed.p.value 

# Getting 95% confidence interval 
pilot.Ys.alt <- genouts(dt.pilot$responded, dt.pilot$treat, ate=pilot.ate)
pilot.distout.alt <- gendist(pilot.Ys.alt, pilot.perms, prob=pilot.probs)
pilot.ci = dispdist(pilot.distout.alt, pilot.ate)$quantile

list(ate = pilot.ate, p = pilot.p, ci = pilot.ci)
```
...
dt.atg[Female == 0, Gender := 'M']
dt.atg[Female == 1, Gender := 'F']
dt.atg[, Y := Responses / N]
```

```{r, results='asis'}
atg.stats = dt.atg[order(Org, Region, Gender, treat), .(Org, Region, Gender, Treat=treat, N)]
